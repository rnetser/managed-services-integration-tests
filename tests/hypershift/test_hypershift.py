...
def oidc_config_id(cluster_parameters, aws_region, rosa_allowed_commands):
    ...
    rosa.cli.execute(
        command=f"create oidc-config --managed=false --prefix {oidc_prefix} --region {aws_region} --mode auto",
    ...
    rosa.cli.execute(
        command=f"delete oidc-config --oidc-config-id {_oidc_config_id} --region {aws_region} --mode auto",
        allowed_commands=rosa_allowed_commands,
    )
